...
- Saves raw XML and processed JSON data to files (optional)
...
def fetch_fpds_data(last_mod_date, agency_code, max_results=10,
                    output_json=None, output_xml=None):
    """
    ...
    Args:
        ...
        output_xml: Optional path to save XML output (for local testing)
    ...
    for page in range(num_pages):
        ...
        # Save raw XML response for reference if it's the first page and output_xml is provided
        if page == 0 and output_xml:
            with open(output_xml, "w", encoding="utf-8") as xml_file:
                xml_file.write(response.text)
            print(f"Raw XML data saved to {output_xml}")
        ...
        try:
            ...
            for entry in entries:
                contract_info = {}
                # Extract ATOM feed elements (title, link, modified, etc.)
                for child in entry:
                    tag = child.tag.split('}')[-1]
                    contract_info[tag] = child.text.strip() if child.text else ""
                    if child.attrib:
                        contract_info[f"{tag}_attributes"] = child.attrib
                
                # Process the <content> element (expected to be XML)
                content = entry.find("{http://www.w3.org/2005/Atom}content")
                if content is not None and content.attrib.get("type") == "application/xml":
                    # Get the raw XML either from content.text or concatenated from child elements
                    if content.text and content.text.strip():
                        raw_xml = html.unescape(content.text.strip())
                    else:
                        raw_xml = "".join(ET.tostring(child, encoding='unicode') for child in list(content))
                        raw_xml = html.unescape(raw_xml)
                    # Clean the XML before parsing
                    cleaned_xml = clean_xml(raw_xml)
                    try:
                        contract_xml = ET.fromstring(cleaned_xml)
                        # Flatten the XML structure into a single dictionary
                        flat_attributes = flatten_xml(contract_xml)
                        # Save all XML details under one key
                        contract_info["award_attributes"] = flat_attributes
                    except ET.ParseError as e:
                        contract_info["content_parse_error"] = f"XML parse error: {e}"
                else:
                    contract_info["content_parse_error"] = "Content element missing or wrong type"
                
                all_contracts.append(contract_info)
            
            # If we've collected enough contracts, stop pagination
            if len(all_contracts) >= max_results:
                all_contracts = all_contracts[:max_results]  # Trim to max_results
                break
...
if __name__ == "__main__":
    ...
    fetch_fpds_data(
        ...
        output_xml="/Users/jasonbonvie/Desktop/fpds_raw_data.xml"
    )